...
async def del_callback(call: types.CallbackQuery):
    ...
    db.delete_goal(user_id, cdict.table_name, cdict.deleted_id)
...
def register_callback_handler(dp: Dispatcher):
    dp.register_callback_query_handler(
        del_callback, text_contains='delete')
    dp.register_callback_query_handler(
        done_callback, text_contains='done')
    dp.register_callback_query_handler(
        cancel_callback, text_contains='cancel')
    dp.register_callback_query_handler(
        show_goals_callback, text_contains='show_goals')
