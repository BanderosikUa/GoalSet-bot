...
async def command_start(message: types.Message):
    await message.answer(get_json_name('english', 'startup'), disable_notification=True)
    await message.answer(get_json_name('english', 'language_setting'), reply_markup=kb_language, disable_notification=True)
...
async def commands(message: types.Message):
    ...
    if message.text == '🇷🇺 russian':
        ...
        await message.answer(message.text, reply_markup=kb_menu('russian'),
                             disable_notification=True)
    elif message.text == '🏴󠁧󠁢󠁥󠁮󠁧󠁿 english':
        db.register(user_id, 'english')
        await message.answer(message.text, reply_markup=kb_menu('english'),
                             disable_notification=True)
    elif message.text == get_json_name(user_lang, 'languages'):
        await message.answer(message.text, reply_markup=kb_language,
                             disable_notification=True)
    elif message.text == get_json_name(user_lang, 'settings'):
        await message.answer(message.text, reply_markup=kb_menu(user_lang),
                             disable_notification=True)
    # --- Menu_details ---
    elif message.text == get_json_name(user_lang, 'menu'):
        await message.answer(message.text, reply_markup=kb_menu_details(user_lang),
                             disable_notification=True)
    ...
    # --- kb_nav ---
    buttons = ['year_goals', 'month_goals', 'week_goals', 'tomorrow_goals', 'today_goals']
    ...
    if message.text in menu_buttons:
        for ind, val in enumerate(menu_buttons):
            if val == message.text:
                ...
                await message.answer(message.text, reply_markup=kb_nav(user_lang),
                                     disable_notification=True)
        ...
        await message.answer(get_json_name(user_lang, 'make_goal_user'),
                             reply_markup=kb_cancel(user_lang), disable_notification=True)
...
# we need to create some password to know what goal we need to delete. Because of
...
async def make_goal(message: types.Message, state: FSMContext):
    ...
    db.make_goal(user_id, table_name, text, str(days_len(table_name)))
    await message.reply('👍', reply_markup=kb_nav(user_lang), disable_notification=True)
    await state.finish()
