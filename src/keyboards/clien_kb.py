...
def kb_menu(user_lang):
    ...
    return ReplyKeyboardMarkup(resize_keyboard=True, ).row(b_menu, b_setting_lang).row(b_statistic)

# --- Menu buttons ---

def kb_menu_details(user_lang):
    b_settings = KeyboardButton(get_json_name(user_lang, 'settings'))
    menu_buttons = get_json_name(user_lang, 'year_goals', 'month_goals', 'week_goals', 'tomorrow_goals', 'today_goals')
    menu_buttons = [KeyboardButton(i) for i in menu_buttons]
    return ReplyKeyboardMarkup(resize_keyboard=True, ).row(menu_buttons[1], menu_buttons[0]).row(menu_buttons[3], menu_buttons[2]).row(menu_buttons[4], b_settings)
...
def kb_nav(user_lang):
    ...
    return ReplyKeyboardMarkup(resize_keyboard=True, ).row(b_add).row(b_back, b_get_goals)
